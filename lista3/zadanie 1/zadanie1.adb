...
-- Bakery algorithm arrays

  --  names as used in the wikipedia article on Lamport's Bakery Algorithm
  type Entering_Array_Type is array (0 .. Nr_Of_Processes - 1) of Boolean;
  Entering : Entering_Array_Type;

  type Number_ArrayType is array (0 .. Nr_Of_Processes - 1) of Integer;
  Number : Number_ArrayType;

  pragma Volatile(Entering);
...
-- Types, procedures and functions
  ...
  --  lock and unlock procedures for bakery algorithm

  function Maximum_Number return Integer is
    Current: Integer := 0;
  begin
  ...
  procedure lock (I : Integer) is
  begin
    --  Put_Line ("process " & Integer'Image(I) & " locking");
    Entering(I) := True;
    ...
    --    while (Number(J) /= 0) and ((Number(J) < Number(I)) or ((Number(J) = Number(I)) and J < I)) loop
    --      null;
    --    end loop;
    --  end loop;
    ...
    for J in Number'Range loop
      if J /= I then
        loop exit when Entering(J) = False; end loop;

        loop exit when (Number(J)=0) or (Number(I)<Number(J)) or (Number(I)=Number(J) and I<J); end loop;
      end if;
    end loop;

    Put_Line ("process " & Integer'Image(I) & " locked");
  end lock;

  procedure unlock (I : Integer) is
  begin
    Number(I) := 0;
    Put_Line ("process " & Integer'Image(I) & " unlocked");
  end unlock;
...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      -- TODO implement the ENTRY_PROTOCOL here ...
      lock(Process.Id);
      ...
      -- TODO implement the EXIT_PROTOCOL here ...
      unlock (Process.Id);
...
begin 
  --  initialize bakery arrays
  for I in Entering'Range loop
    Entering(I) := False;
    Number(I) := 0;
  end loop;
